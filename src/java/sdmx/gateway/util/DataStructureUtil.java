...
public class DataStructureUtil {
    ...
    public static sdmx.gateway.entities.DataStructure createDatabaseDataStructure(EntityManager em, DataStructureType ds) {
        ...
        List<sdmx.gateway.entities.DataStructureComponent> comps = new ArrayList<sdmx.gateway.entities.DataStructureComponent>();
        int position = 0;
        ...
        if (ds.getDataStructureComponents().getDimensionList().getTimeDimension() != null) {
            comps.add(DimensionUtil.toDatabasetTimeDimension(em, ds, ds.getDataStructureComponents().getDimensionList().getTimeDimension(), position++));
        }
        ...
        for (int i = 0; i < ds.getDataStructureComponents().getAttributeList().size(); i++) {
            comps.add(DimensionUtil.toDatabaseAttribute(em, ds, ds.getDataStructureComponents().getAttributeList().getAttribute(i), position++));
        }
        comps.add(DimensionUtil.toDatabasePrimaryMeasure(em, ds, ds.getDataStructureComponents().getMeasureList().getPrimaryMeasure(), position++));
